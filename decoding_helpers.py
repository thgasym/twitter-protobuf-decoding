...
def try_decode_pb_array(name, buf, decoder, verbose=True):
    ...
    # just a canary, likely will fail so silence error reporting
    res = decoder(buf, False)
    # print("first try: "+res)
    if res == "Failed":
...
def decode_pb_array(name, buf, decoder, verbose=False):
    ...
    while (pos < len(buf)):
        ...
        if (res == "Failed"):
            ...
            print("***decode_pb_array: ", pos, pos +
                  new_pos, pos+new_pos+msg_len, len(buf))
            # try raw decoding of protobuf, maybe schema mismatch
            res = decode_pb(bytes, verbose)
            if (res == "Failed"):
...
def decode_checkin(bytes, verbose=False):
    try:
        ...
        f.write(bytes)
        f.close()
        decoded = subprocess.check_output("protoc --decode=\"CheckinRequest\" -I='"+mypath +
                                          "' checkin.proto  </tmp/checkin_bytes", shell=True, stderr=subprocess.STDOUT, text=True)
        # print(decoded)
        return (decoded)
    except subprocess.CalledProcessError as e:
        if verbose:
